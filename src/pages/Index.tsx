  ...
  const filtered = useMemo(() => {
    return samplePosts.filter((post) => {
      ...
      const matchSearch =
        ...
        post.titleHi.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="relative h-[70vh] min-h-[400px] flex items-center justify-center overflow-hidden">
        <img src={heroBanner} alt="Rajput Fort" width={1920} height={800} className="absolute inset-0 w-full h-full object-cover" />
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            ...
            <a href="#posts">
              <Button variant="outline" className="border-gold/50 text-primary-foreground bg-gold/10 hover:bg-gold/20 px-6 py-3">
                {t("hero.browse")} <ChevronRight size={16} className="ml-1" />
              </Button>
            </a>
          </div>
        </div>
      ...
      <main id="posts" className="container mx-auto px-4 py-12 flex-1">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h2 className="font-heading text-2xl md:text-3xl font-bold text-foreground">{t("posts.latest")}</h2>
            <p className="text-muted-foreground text-sm mt-1">{t("posts.subtitle")}</p>
          </div>
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-16">
            <p className="text-muted-foreground text-lg">{t("posts.noResults")}</p>
          </div>
        )}
      </main>
